...
then
    ...
    if [ "$2" != "NA" ] && [ "$3" != "NA" ]
    then
        bcftools annotate -x $2 $FILEIN -Ob | bcftools norm -m - -Ob | bcftools view -f PASS -Ob | bcftools filter -i $3 -Oz -o $FILEOUT --threads $4
    elif [ "$2" != "NA" ] && [ "$3" == "NA" ]
    then
        bcftools annotate -x $2 $FILEIN -Ob | bcftools norm -m - -Ob | bcftools view -f PASS  -Oz -o $FILEOUT --threads $4
    else
        bcftools norm -m - $FILEIN -Ob | bcftools view -f PASS -Ob | bcftools filter -i $3 -Oz -o $FILEOUT --threads $4
    fi
