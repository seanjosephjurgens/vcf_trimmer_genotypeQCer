...
then
    ...
    then
        ...
        bcftools annotate -x $2 $FILEIN -Ou | bcftools norm -m - -Ou | bcftools view -f PASS -Ou | bcftools +setGT -Ou -- -t q -i 'FORMAT/FT!="PASS" && FORMAT/FT!="." && FORMAT/FT!=""' -n . | bcftools filter -i $3 -Oz -o $FILEOUT --threads $4
    ...
    then
        export BCFTOOLS_PLUGINS=plugins/
        bcftools annotate -x $2 $FILEIN -Ou | bcftools norm -m - -Ou | bcftools view -f PASS -Ou | bcftools +setGT -Oz -o $FILEOUT --threads $4 -- -t q -i 'FORMAT/FT!="PASS" && FORMAT/FT!="." && FORMAT/FT!=""' -n .
        #bcftools annotate -x $2 $FILEIN | bcftools norm -m - | bcftools view -f PASS | bcftools -Oz -o $FILEOUT --threads $4
        #export BCFTOOLS_PLUGINS=plugins/
        #bcftools +setGT $FILEOUT --threads $4 -- -t q -i 'FORMAT/FT!="PASS"' -n .
    else
        export BCFTOOLS_PLUGINS=plugins/
        bcftools norm -m - $FILEIN -Ou | bcftools view -f PASS -Ou | bcftools +setGT -Ou -- -t q -i 'FORMAT/FT!="PASS" && FORMAT/FT!="." && FORMAT/FT!=""' -n . | bcftools filter -i $3 -Oz -o $FILEOUT --threads $4
